...
def create_subscriber(event, context):
    ...
    response = manheim_session.post(subscriber_url, json=subscriber_data)
...
def update_subscriber(event, context):
    ...
    response = manheim_session.post(physical_resource_id, json=subscriber_data)
...
def delete_subscriber(event, context):
    ...
    if physical_resource_id.scheme is not None:
        params = {"force": "true"}
        manheim_session.delete(physical_resource_id.geturl(), params=params)
